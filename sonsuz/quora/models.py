...
class QuestionQuerySet(models.query.QuerySet):
    ...
    def get_correct_answered(self):
        """已有正确答案的问题"""
        return self.filter(has_correct=True).select_related('user')

    def get_uncorrect_answered(self):
        """未被正确回答的问题"""
        return self.filter(has_correct=False).select_related('user')
...
class Question(models.Model):
    ...
    def get_accepted_answer(self):
        """被接受的回答"""
        return Answer.objects.select_related('user', 'question').get(question=self, is_accepted=True)

    def get_upvoters(self):
        """赞同的用户"""
        # return [vote.user for vote in self.votes.filter(value=True).select_related('user').prefech_related('vote')]
        return [vote.user for vote in self.votes.filter(value=True).select_related('user')]

    def get_downvoters(self):
        """反对的用户"""

        # return [vote.user for vote in self.votes.filter(value=False).select_related('user').prefech_related('vote')]
...
class Answer(models.Model):
    ...
    content = MDTextField(verbose_name='答案内容')
