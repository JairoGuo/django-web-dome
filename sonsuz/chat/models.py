...
class MessageQuerySet(models.query.QuerySet):
    ...
    def get_conversation(self, sender, reciever):
        """用户间的私信会话"""
        qs_one = self.filter(sender=sender, reciever=reciever).select_related('sender', 'reciever')  # A发送给B的消息
        qs_two = self.filter(sender=reciever, reciever=sender).select_related('sender', 'reciever')  # B发送给A的消息
        return qs_one.union(qs_two).order_by('created_at')  # 取并集后按时间排序

    def get_most_recent_conversation_user(self, reciever):
        """获取最近一次私信互动的用户"""
        try:
            qs_sent = self.filter(sender=reciever).select_related('sender', 'reciever')  # 当前登录用户发送的消息
            qs_received = self.filter(reciever=reciever).select_related('sender', 'reciever')  # 当前登录用户接收的消息
...
class Message(models.Model):
    ...
    created_at = models.DateTimeField(auto_now_add=True, verbose_name='创建时间', db_index=True)  # 没有updated_at，私信发送之后不能修改或撤回
