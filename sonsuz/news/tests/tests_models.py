...
class NewsModelsTest(TestCase):
    ...
    def setUp(self):
        ...
        self.user2 = self.make_user(username="user2", password="ooooo")
        self.news1 = News.objects.create(user=self.user1, content="第一条新闻")
        self.news2 = News.objects.create(user=self.user1, content="第二条新闻")
        self.news3 = News.objects.create(user=self.user2, content="回复第一条新闻",
                                         reply=True, parent=self.news1)
    ...
    def test_reply_this(self):
        ...
        assert self.news1.replies_count() == 1
